...
class FillingLayout: UICollectionViewLayout {
    ...
    fileprivate var contentWidth: CGFloat {
        ...
        return collectionView.bounds.inset(by: collectionView.layoutMargins).size.width - (insets.left + insets.right)
    }
        ...
        for item in 0 ..< collectionView.numberOfItems(inSection: 0) {
            ...
            if (viewSize > 1) {//Big Cell
                ...
                if (self.avaiableSpaces.count == 0) {
                    ...
                    //if there is some avaiable space
...
/*extension NewsVC: FillingLayoutDelegate {
 
 func collectionView(_ collectionView: UICollectionView, sizeForViewAtIndexPath indexPath: IndexPath) -> Int {
 if (indexPath.row == 0 || indexPath.row == 4 || indexPath.row == 6 || indexPath.row == 10 || indexPath.row == 11 || indexPath.row == 16 || indexPath.row == 18 ) {
 return 2
 }
 return 1
 }
